...
__uninitialized_copy_aux(_InputIter __first, _InputIter __last,
                         ...
                         __true_type)
{
  return copy(__first, __last, __result); //调用STL方法copy()
}
                         ...
                         __false_type)
{
  ...
  __STL_TRY {
    for ( ; __first != __last; ++__first, ++__cur)
      construct(&*__cur, *__first);   //必须一个一个的构建，无法批量进行
    return __cur;
  }
...
//对于char*和wchar_t*类型可用最有效率的方法memmove直接移到内存数据来执行复制
...
__uninitialized_fill_n_aux(_ForwardIter __first, _Size __n,
                           const _Tp& __x, __true_type)
{
  return fill_n(__first, __n, __x);     //交由高价函数执行
}
