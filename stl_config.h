...
//定义各种常量，主要是为编译服务的
//由于不同的编译环境对标准C++的支持不完全相同，出于移植性的考虑，SGI STL定义了这个文件。
//它通过条件编译针对不同的编译器作了部分常数设定
...
//如果支持类模板局部特殊化,也名类模板的偏特化(partial specialization of class templates),则定义 
...
//如果编译器允许我们在调用一个函数模板时指定它的模板实参，则定义
//   supports calling a function template by providing its template
//   arguments explicitly.
//如果编译器无法根据前一个模板形参设置之后的参数，则定义 
...
//   parameters.
//如果编译器针对非类类型的模板参数执行参数推断(argument deductioin)时有问题，则定义
...
//如果编译器(在你所选择的模式中)支持exceptions，则定义
...
//   pthreads.
//如果程序由一个win32编译器编译，并且在多线程模式下，则定义
